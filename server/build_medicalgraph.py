...
class MedicalGraph:
    def __init__(self):
        self.g = Graph('neo4j://localhost:7687', user="neo4j", password="test1234")
    ...
    def read_nodes(self):
        ...
        for index, row in df.iterrows():
            ...
            symptom_temp = row['symptom'].replace(
                '[', '').replace(']', '').replace("'", '').split(",")
            symptoms += symptom_temp
            for symptom in symptom_temp:
                rels_symptom.append([disease, symptom])

            accompany_temp = row['accompany'].replace(
                '[', '').replace(']', '').replace("'", '').split(",")
    ...
    def create_graphrels(self):
        ...
        self.create_relationship(
            'Disease', 'Disease', rels_acompany, 'accompany_with', 'complication')
        self.create_relationship(
            'Disease', 'Department', rels_category, 'disease_belongs_to', 'department')
    ...
    def export_data(self):
        ...
        f_department = open('/Data/dict/department.txt', 'w+')
        f_symptom = open('/Data/dict/symptoms.txt', 'w+')
        f_disease = open('/Data/dict/disease.txt', 'w+')
